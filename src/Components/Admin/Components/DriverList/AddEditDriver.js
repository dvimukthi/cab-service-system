...
function AddEditDriver({open, id, status,setStatus,setId, setOpen, setReload}) {
  ...
  const updateDriver = async () => {
    ...
    const response = await axios.put(`http://localhost:3001/driver/${id}`,data);
      ...
      <Dialog open={open} onClose={handleClose}>
        ...
        <form className="DriverForm__Container">
          <div className="Register__Form">
            ...
            <div className="DriverBtn" onClick={saveDriver}>{currStatus}</div>
          </div>
        </form>
        <DialogActions>
